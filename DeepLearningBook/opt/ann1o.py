...
class NN:
    ...
    def lmodelfwd(self, X):
        ...
        for l in range(1, L):
            ...
            if (self.streg == 'dout'):
                ...
                mask = np.random.binomial(1, self.stld, size=A.shape) / self.stld
                masks['m' + str(l)] = mask
            else:
                A, cache = self.linactfwd(A_prev, self.parameters['W' + str(l)], self.parameters['b' + str(l)], self.actis[l])
            caches.append(cache)
    ...
    def linactback(self, dA, cache, act):
        ...
        if act == "relu":
            ...
            dZ = self.dlinear(dA, activation_cache)
            dA_prev, dW, db = self.linback(dZ, linear_cache)
        return dA_prev, dW, db
    ...
    def updateparams(self, grads, ldbym):
        ...
        for l in range(L):
            if (self.reg == 'l2reg'):
                self.parameters["W" + str(l + 1)] = (1 - self.learning_rate *ldbym) * self.parameters["W" + str(l + 1)] - self.learning_rate * grads[
                    "dW" + str(l + 1)]
            elif (self.reg == 'l1reg'):
                self.parameters["W" + str(l + 1)] = self.parameters["W" + str(l + 1)] - self.learning_rate * grads[
                    "dW" + str(l + 1)] - self.learning_rate *ldbym * np.sign(self.parameters["W" + str(l + 1)])
            else:
...
class bagging_NN:
    def __init__(self, k, layers, actis, hyperparams):
        self.bags = {}
        self.models = {}
        self.k = k
        np.random.seed(3)
    ...
    # defining bags
    def make_bags(self, x, y):
        m = y.shape[1]
        np.random.seed(12)
        for i in range(self.k):
            permutation = np.random.randint(0, m - 1, m) + (np.random.randn(1, m) > 0)
            self.bags['bag_x' + str(i)] = x[:, permutation]
            self.bags['bag_y' + str(i)] = y[:, permutation]


    def train_bagging(self, x_str, y_str, x_val, y_val):
        self.make_bags(x_str, y_str)
        for i in range(self.k):
            print('Training bag no.' + str(i))
            x_trn = self.bags['bag_x' + str(i)].reshape(x_str.shape[0], x_str.shape[1])
            y_trn = self.bags['bag_y' + str(i)].reshape(y_str.shape[0], y_str.shape[1])
            self.models['m' + str(i)] = NN(self.layers, self.actis, self.hyperparams)
            self.models['m' + str(i)].train(x_trn, y_trn, x_val, y_val)

    def predict_bagging(self, x):
        k = len(self.models)
        pY = 0
        for i in range(k):
            pY = pY + self.models['m' + str(i)].predict(x)
        pY = pY / k
        return pY